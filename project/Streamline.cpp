...
std::vector<Eigen::Vector3d> Streamline::getSeeds(double sepDist) {
	...
	seeds.push_back(cartToSph(cart0 + cart0.normalized() * (sepDist / RADIAL_DIST_SCALE)));
	seeds.push_back(cartToSph(cart0 - cart0.normalized() * (sepDist / RADIAL_DIST_SCALE)));
	...
	for (size_t i = 1; i < size() - 2; i+=50) {
		...
		seeds.push_back(cartToSph(cart + cart.normalized() * (sepDist / RADIAL_DIST_SCALE)));
		seeds.push_back(cartToSph(cart - cart.normalized() * (sepDist / RADIAL_DIST_SCALE)));
	}
	...
	seeds.push_back(cartToSph(cartE + cartE.normalized() * (sepDist / RADIAL_DIST_SCALE)));
	seeds.push_back(cartToSph(cartE - cartE.normalized() * (sepDist / RADIAL_DIST_SCALE)));
